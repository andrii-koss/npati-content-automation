...
(async () => {
  ...
  try {
    ...
    if (!await page.locator('[name="disableBuy"]').evaluate(node => !node.disabled && node.checked) || !await page.locator('[name="disableBuy"]').locator('..').evaluate(node => node.classList.contains('is-locked') && getComputedStyle(node).pointerEvents === 'none')) throw new Error('Disable Buy must be visually unchanged and locked in the enabled state');
    if (!await page.locator('[name="shippingAvailable"]').evaluate(node => !node.disabled && !node.checked) || !await page.locator('[name="shippingAvailable"]').locator('..').evaluate(node => node.classList.contains('is-locked') && getComputedStyle(node).pointerEvents === 'none')) throw new Error('Delivery must be visually unchanged and locked in the disabled state');
